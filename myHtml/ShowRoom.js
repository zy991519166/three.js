...
//加载贴图结束
function loadTex(){
	const pictures = scene.getChildByName("Pics");
					...
					texture = new THREE.TextureLoader().load(
						...
						function ( texture ) {
							...
							pictures.children[i].material = new THREE.MeshStandardMaterial( {
								...
								emissive:0x555555,
								emissiveMap: texture} );
							pictures.children[i].material.map.anisotropy = 4;
						},
						undefined,
						undefined)
				}
...
loader.load( 'scene.glb', ( gltf ) => {
	...
	const  lightMap = new THREE.TextureLoader().load('showroom/textures/FloorLightingMap.jpg');
...
function raycast(){
	...
	castRay = false;
	if (intersects.length > 0 && (intersects[0].object.name=='Floor' || intersects[0].object.name=='2F')) {
